...
class CustomerProfileView(View):  
    def get(self, request):
        ...
        return render(request,'eshop_accounts/user_account_main.html',{"user":customer.user})   
...
class SignUpSuplier(FormView):
    ...
    def form_valid(self, form):
        suplier = form.save()
        self.send_mail(request, suplier.user.email)
...
class CreateProductView(CreateView):
    ...
    def form_valid(self, form):
        suplier = Suplier.objects.filter(user=self.request.user)
        if suplier:
            form.instance.suplier = suplier
            return super().form_valid(form)
        raise PermissionDenied('nemitoony')
